...
-- Path resolution
function ModuleLoader:ResolveRelativePathFromCurrentScript(relativePathFromCurrentScript)

end

-- IsBundle

-- Lua/C module loading
function ModuleLoader:LoadLuaModuleFromDisk(absoluteFileSystemPath)

end

function ModuleLoader:LoadLuaModuleFromPackageFolder(relativePathFromBundleRoot)

end

function ModuleLoader:LoadLuaModuleFromVirtualFileSystem(relativePathFromBundleRoot)

end

function ModuleLoader:LoadNativeModuleFromBundle(relativePathFromBundleRoot)
...
-- Legacy
ModuleLoader.EVO_PACKAGE_DIRECTORY = ".evo"
...
-- This can only be done at runtime since the path extension isn't yet loaded when this chunk is read
...
function ModuleLoader:ImportModule(modulePath)
	...
	if string_sub(modulePath, 1, 1) == "@" then
		...
		modulePath = path_join(self.EVO_PACKAGE_DIRECTORY, strippedModulePath)
	...
	local parentModule = (#self.prefixStack == 0) and _G.USER_SCRIPT_PATH or self.prefixStack[#self.prefixStack]
	...
	local cachedModule = self.moduleCache[absolutePath]
	...
	self.prefixStack[#self.prefixStack+1] = absolutePath
	...
	elseif vfs.hasFolder(path_join(self.EVO_PACKAGE_DIRECTORY, modulePath)) then
		modulePath = path_join(self.EVO_PACKAGE_DIRECTORY, modulePath, "main.lua")
...
-- EvoPackageLoader
-- VirtualFileSystemLoader
-- DefaultFileSystemLoader

-- IsEvoPackagePath
-- IsBundleContext
-- SetEntryPoint(absoluteEntryPointPath)

-- C land -> Lua land (runtime) -> Lua land (bundle/vfs OR disk) -> user script (disk)