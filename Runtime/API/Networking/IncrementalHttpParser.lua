...
function IncrementalHttpParser:ParseChunkAndRecordCallbackEvents(chunk)
    ...
    local writableBufferArea = ffi_cast("luajit_stringbuffer_reference_t*",
                                        self.state.data)
...
function IncrementalHttpParser:IsOK()
    ...
    local isUpgradeRequest = (llhttpErrorCode ==
                                 llhttp.ERROR_TYPES.HPE_PAUSED_UPGRADE) -- Not really an error...
...
for index, readableEventName in pairs(llhttp.FFI_EVENTS) do
    IncrementalHttpParser[readableEventName] =
        function(parser, eventID, payload)
