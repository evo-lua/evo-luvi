...
do
	...
	for i, part in ipairs(original_parts) do
		p("part", part)
		local deflated, err, partial = deflator:deflate(part, i == #original_parts and "finish" or "sync")
		p("compressed", deflated, partial)
		deflated = assert(not err, err) and (deflated or partial)
		local inflated, err, partial = inflator:inflate(deflated, i == #original_parts and "finish" or "sync")
