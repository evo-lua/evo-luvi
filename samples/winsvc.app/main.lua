...
local function ReportSvcStatus(dwCurrentState, dwWin32ExitCode, dwWaitHint)
	...
	if dwCurrentState == winsvc.SERVICE_RUNNING or dwCurrentState == winsvc.SERVICE_STOPPED then
...
local function SvcInstall()
	...
	winsvc.ChangeServiceConfig2(
		...
		{ lpDescription = "This is a test service written in Luvi/Lua" }
	)
...
local function SvcDelete()
	...
	-- Open the Service
	local schService, err = winsvc.OpenService(schSCManager, svcname, winsvc.DELETE)
...
local function SvcStart()
	...
	-- Open the Service
	local schService, err = winsvc.OpenService(schSCManager, svcname, winsvc.SERVICE_START)
...
local function SvcStop()
	...
	-- Open the Service
	local schService, err = winsvc.OpenService(schSCManager, svcname, winsvc.SERVICE_STOP)
